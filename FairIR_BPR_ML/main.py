...
import os
import argparse
parser = argparse.ArgumentParser(description='FairIR BPR Model Training')
parser.add_argument('--K', type=int, default=120, help='Number of negative samples')
parser.add_argument('--tau', type=float, default=1.0, help='Temperature parameter for InfoNCE loss')
parser.add_argument('--model_name', type=str, default='FairIR_BPR_ML', help='Base model name')
...
run_id = f"{args.model_name}_k{args.K}_tau{args.tau:g}"
print('Model:',run_id)
dataset='ml'
learning_rate = 1e-3  
...
class FairIR(nn.Module):
    ...
    def noise_pos(self,male_noise_i_emb, female_noise_i_emb):    
    ...
    def forward(self, adj_pos,u_batch,i_batch,j_batch):
        ...
        male_like_emb, female_like_emb = self.noise_pos(male_noise_i_emb,female_noise_i_emb)
        ...
        loss_info_nce_male = FairIR.info_nce_loss(male_u_emb, male_like_emb, male_neg_emb, tau)
        loss_info_nce_female = FairIR.info_nce_loss(female_u_emb, female_like_emb, female_neg_emb, tau)
...
############ Model #############
model = FairIR(user_num, item_num, factor_num,users_features)
model=model.to('cuda') 
...
for epoch in range(1001):
    ...
    if  (epoch % 10 == 0) :
        str_print_train = f"epoch:{epoch} time:{round(elapsed_time, 1)} loss task:{train_loss_task}  loss InfoNCE:{train_loss_noise}"
    ...
    if   (epoch % 25 == 0 and epoch > 900) :
        ...
        print(f"Training Time: {round(total_training_time/60, 2)}mins ({round(total_training_time, 2)}s)")
