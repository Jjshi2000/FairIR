...
def gender_metrics(top_k):
    # print(f'--------gender-based metrics processing, Top K: {top_k}-------')
    for epoch in ['best']:
        PATH_model_u = path_save_model_base + '/user_emb.npy'
        user_e = np.load(PATH_model_u)
        PATH_model_i = path_save_model_base + '/item_emb.npy'
        ...
        HR_male_num, NDCG_male_num = 0, 0
        HR_female_num, NDCG_female_num = 0, 0
        ...
        for idx_pre_s,idx_pre_e in idx_all:
            ...
            # Split metrics by gender
            gender_slice = users_features[idx_pre_s:idx_pre_e]
            male_mask = gender_slice == 1
            female_mask = gender_slice == 0
            
            # Calculate HR for each gender
            hr_t1 = hr_topK/max_hr
            hr_t1_valid = hr_t1[~np.isnan(hr_t1)]
            gender_slice_valid = gender_slice[~np.isnan(hr_t1)]
            
            male_hr = hr_t1_valid[gender_slice_valid == 1]
            female_hr = hr_t1_valid[gender_slice_valid == 0]
            
            # Calculate NDCG for each gender
            ndcg_t = ndcg_topK/max_ndcg
            male_ndcg = ndcg_t[male_mask]
            female_ndcg = ndcg_t[female_mask]
            
            # Accumulate metrics
            HR_male += male_hr.sum()
            HR_female += female_hr.sum()
            NDCG_male += male_ndcg.sum()
            NDCG_female += female_ndcg.sum()
            
            HR_male_num += len(male_hr)
            HR_female_num += len(female_hr)
            NDCG_male_num += len(male_ndcg)
            NDCG_female_num += len(female_ndcg)
            
        # Calculate final metrics
        hr_male = round(HR_male/HR_male_num, 4)
        hr_female = round(HR_female/HR_female_num, 4)
        ndcg_male = round(NDCG_male/NDCG_male_num, 4)
        ndcg_female = round(NDCG_female/NDCG_female_num, 4)
        
        print(f"Male - HR: {hr_male} NDCG: {ndcg_male}")
        print(f"Female - HR: {hr_female} NDCG: {ndcg_female}")   
