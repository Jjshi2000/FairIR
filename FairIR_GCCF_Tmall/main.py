# python main.py --gpu 2 --k 40 --tau 0.1 --len_noise 0.3 > output.txt
import os
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('--gpu', type=int, default=5, help='GPU ID')
parser.add_argument('--k', type=int, default=30, help='K值 (10-100)')
parser.add_argument('--tau', type=float, default=0.6, help='温度参数 (0.1-2.0)')
parser.add_argument('--len_noise', type=float, default=0.3, help='噪声长度比例 (0-1)')
...
dataset_base_path='../data/tmall' 


user_num=34614#user_size
item_num=21304#item_size 
factor_num=64
...
class GCN(nn.Module):
    ...
    def forward(self, u_batch, i_batch, j_batch):
        ...
        male_neg_emb = male_j_emb[male_neg_indices]  # 用于 male_j_emb 的负采样

        female_neg_indices = torch.randint(0, female_j_emb.size(0), (female_j_emb.size(0), K), device=female_j_emb.device)
        female_neg_emb = female_j_emb[female_neg_indices]  # 用于 female_j_emb 的负采样
        ...
        loss_fake = 0.1*loss_info_nce_male +0.1*loss_info_nce_female  + loss_fake_male +  loss_fake_female
...
testing_dataset_loss = data_utils.BPRData(
    ...
    num_ng=5,
    is_training=1,
    data_set_count=test_dict_count
)
testing_loader_loss = DataLoader(
    testing_dataset_loss,
    batch_size=test_dict_count,
    shuffle=False,
    num_workers=0
)
